...
class Organization extends Component {
  ...
  render() { 
    const { OrganizationTableData } = this.props.state
    const columns =  [
      {
        title: '排名',
    ...
    return (
      <div className='AddRank'>
        <div className='chartBlock' style={{borderBottom:'1px solid #EEEEEE',marginBottom: '50px'}}>
          <Chart height={400} data={OrganizationTableData.chartData} forceFit>
        ...
        <div className='btnBlock' style={{marginBottom: '30px'}}>
          ...
          <Button className='lessBtn'><i className='iconfont dangjiankaohe_icon_yijiantixing'></i>一键提醒</Button>
        </div>
        <div className='tableBlock'>
          <Table style={{marginBottom:'20px'}} loading={OrganizationTableData.loading} dataSource={OrganizationTableData.list} columns={columns} pagination={false}/> 
        </div>
      </div>
    );
  }
