...
@connect
class DownClass extends Component {
  ...
  render() {
          ...
          return (
            !record.edit?(
              <span>
                ...
                <i className='iconfont dangjiankaohe_icon_shanchu' onClick={()=>this.delete({index:index,id:record.hand_id,type:3})}></i>
              </span>
            ):(
              <span>
                <Button className='saveBtn' onClick={()=>handleSaveEditBtnClick(3,{index:index,record:record})}>保存</Button>
    ...
    return (
      <Fragment>
        ...
        <Table columnsData ={columns} initFuc={(e)=>handscoreInit(3,e)} batchDeleteFuc={()=>this.batchDelete(tableData.list,tableData.selectedRowKeys,3)} /> 
        <Alert />
      </Fragment>
    );
  }
