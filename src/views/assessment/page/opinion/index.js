...
class opinion extends React.Component {
    ...
    componentDidMount = () => {
        axios.get('v1/appraise/preliminaryOpinion', { page: 1, limit: '10' })
            .subscribe(res => {
                this.setState({
                    isopen: false,
                    time: null,
    ...
    columns = [
        ...
        },
        {
            ...
            render: (is_check, record) => (
                is_check == 0 ? <div class="self" onClick={() => this.goEdit(record.id)}>初评意见</div> :
                    <div class="view" onClick={() => this.goEdit(record.id)}>查看意见</div>
            )
        },
    ];

    goEdit(id) {
        this.props.history.push({ pathname: `/assessment/opinionEdit/${this.state.year}/${id}` })
    ...
    initData = (page, year) => {
        axios.get('v1/appraise/preliminaryOpinion', { page: page, limit: 10, year: year })
            .subscribe(res => {
                this.setState({
                    data: res.data.data,
                    loading: false,
